...
<table class="table table-bordered table-hover">
    ...
    @forelse ($wishlists as $wishlist)
        <tr>
            <td>{{ $loop->index + 1 }}</td>
            <td>
                <img src="{{ asset($wishlist->product->info->image) }}" alt="" width="100px">
            </td>
            <td>
                <a class="text-success" href="{{ route('single-product',$wishlist->product->slug) }}">{{ $wishlist->product->name }}</a>
            </td>
            <td>{{ $wishlist->created_at->diffForHumans() }}</td>
            <td class="text-center">
            <form action="{{ route('add-to-cart',$wishlist->product) }}" method="POST" class="d-inline">
                    @csrf
        ...
        @empty
        <tr>
            <td colspan="5" class="text-center"><span class="text-danger">Empty!!</span></td>
        </tr>
        @endforelse
</table>
@stop
